...
export function TopTitleBar({
  ...
  return (
    <section
      className="w-full bg-[#1A1A1A] border-b-[2px] border-[#141B16] px-3 py-1 flex items-center justify-between gap-2 z-30 flex-shrink-0 text-white font-['Rowdies'] shadow-md h-[34px] min-h-[34px] overflow-hidden"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1.5 bg-[#16432D] border border-[#4ADE80]/40 px-2 py-0.5 rounded-[4px] text-[10px] shadow-sm">
          <span className="text-[#E5B842] font-bold tracking-wide">HERITAGE</span>
          <span className="text-white/80 font-light text-[9px]">Vertical Cedar Picket</span>
        </div>

        {/* Quick Jump Dropdown Triggers */}
        <div className="hidden xl:flex items-center gap-1.5 text-xs text-white/60">
          <button
            onClick={() => onSelectChapter?.('height')}
            className="px-2 py-0.5 bg-[#141B16] hover:bg-[#253328] hover:text-[#E5B842] border border-white/10 rounded transition text-[9px]"
          >
      ...
      <div className="flex items-center gap-2 text-[9px] text-white/50 uppercase font-bold">
        <div className="flex items-center gap-1">
          <span>View:</span>
          <select
            value={viewAngle}
            onChange={(e) => onViewAngleChange(e.target.value as any)}
            className="bg-transparent border-none text-[#E5B842] focus:ring-0 cursor-pointer text-[9px] p-0 font-bold uppercase"
          >
            <option value="both" className="bg-[#1C241E]">Dual</option>
            <option value="front" className="bg-[#1C241E]">Front</option>
            <option value="back" className="bg-[#1C241E]">Back</option>
          </select>
        </div>

        <div className="flex items-center bg-[#141B16] border border-white/15 rounded p-0.5 text-[9px] text-white">
          <button
            onClick={() => handleZoom(-0.1)}
            className="px-1.5 py-0 hover:bg-white/10 rounded font-bold"
            title="Zoom out"
          >
            -
          </button>
          <span className="px-1 text-white/80 text-[8px] min-w-[2.2rem] text-center font-mono">
